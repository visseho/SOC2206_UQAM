...
**Faux, il vaut 1. C'est l'écart-type qui vaut 1.**

3. On peut calculer les scores-Z pour les variables ordinales (Vrai ou Faux)

**Faux**

4. Si vous prenez deux distributions normales A et B avec A plus applatie (signifie quoi) que l'autre, alors la proportion des scores qui se trouve à 1,5 écart-type de la moyenne de A est plus grande que la proportion des scores qui se trouve à 1,5 écart-type de la moyenne de B (Vrai ou Faux)
...
<!-- Cette partie est un travail en développement. Vous ne devez pas le lire

# Représentation graphique
Si nous supposons que les échantillons en questions sont tirés de la population de toutes les voitures, d'après le théorème centrale limite, nous pouvons dire que ces moyennes sont distribués de manière normale avec pour moyenne la moyenne obtenue et et erreur-type, l'écart-type divisé par la racine carré de N


```{r}

erreur_type_1994 <- ecart_1994/sqrt(18)
erreur_type_1994


ggplot(data = data.frame(x = c(15, 35)), aes(x)) +
  geom_function(fun = dnorm, args = list(mean = Tendance_1994[1, 2], sd = erreur_type_1994[1]), color = "red") +
  geom_function(fun = dnorm, args = list(mean = Tendance_1994[2, 2], sd = erreur_type_1994[2]), color = "blue")
...
l'écart-type est ce que vous calculer à partir d'un seul échantillon, alors que l'erreur-type est ce que vous calculer à partir d'une distribution d'échantillonnage. Donc, en haut, j'ai plutôt calculé les erreurs-types.
