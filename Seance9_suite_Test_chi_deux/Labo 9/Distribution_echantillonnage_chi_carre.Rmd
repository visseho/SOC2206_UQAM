...
cis <- read.csv("cis_short_5percent_2015.csv")

```


# 100 échantillons de taille 200

```{r}

cis_T200_R100 <- bind_rows(replicate(100, cis %>% sample_n(200, replace = FALSE), simplify = F), .id = "Echantillon")

ctable(cis$region, cis$sexe, chisq = TRUE)
...
cis_T200_R100 <- bind_rows(replicate(100, cis %>% sample_n(200, replace = FALSE), simplify = F), .id = "Obs")
